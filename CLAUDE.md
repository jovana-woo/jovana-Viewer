...
- 두 장 보기에서 좌/우 페이지를 독립적으로 확대/유지 가능 (`zoomLeft`, `zoomRight`)
- 클릭 확대는 페이지(좌/우)당 1회 제한, 더블클릭으로 원본(페이지 맞춤) 복귀
- 읽기 방향 버튼(`우→좌`/`좌→우`) 설정이 재실행 후에도 유지됨
- 야간 모드(`☾`) 추가: 윈도우 야간 모드 유사한 웜(노란) 톤, 설정 유지
- 전체화면에서 3초 무입력 시 커서 자동 숨김, 마우스 이동 시 즉시 표시
- 페이지 이동 후에도 전체화면 커서 숨김 상태가 유지되도록 보정
- 탐색기 경로바 아래 검색창 추가(폴더/파일 실시간 필터)
- 툴바와 제목줄 중앙 정렬, 버튼 순서/간격 재정리
- About(ⓘ) 모달 추가 및 제작자 표기 `Jovana/조동연/` 반영
- zip 진행상황 복원 강화(내부 폴더 prefix 포함), 배포 문서/라이선스 추가
...
```js
{
  ...
  zoom: 1.0,          // 단일 보기 또는 공통 표시용 줌 값
  zoomLeft: 1.0,      // 두 장 보기 왼쪽 페이지 개별 줌
  zoomRight: 1.0,     // 두 장 보기 오른쪽 페이지 개별 줌
  zoomTarget: 'left', // 클릭 확대 대상('left'|'right')
...
- **탐색기 검색창**: 탐색기 경로바 아래 검색 입력으로 폴더/파일 즉시 필터링
...
- **추가 키보드**: Home/End 이동, Space/Backspace 이동, Esc 전체화면 해제, Enter 전체화면 토글
- **마우스 클릭**:
  - 단일 클릭: 현재 포인트 중심으로 1.35x 확대
  - 클릭 확대는 페이지(좌/우)당 1회 제한
  - 두 장 보기에서는 좌/우를 독립적으로 확대 유지 가능
...
- **전체화면 커서 자동 숨김**: 전체화면에서 3초 무입력 시 커서 숨김, 움직이면 즉시 표시
- **야간 모드**:
  - 툴바 `☾` 버튼으로 토글
  - 윈도우 야간 모드 유사한 노란(웜) 톤 필터 적용
  - 설정은 localStorage에 저장되어 앱 재실행 후에도 유지
- **정보 모달(About)**:
  - 툴바 `ⓘ` 버튼으로 열기
  - 제작자 표기: `Jovana/조동연/`
  - 저작권 및 상업적 무단 배포 금지 안내 포함
- **zip 진행상황 복원 강화**: zip 내부 마지막 폴더(prefix)까지 저장/복원
...
- `renderer.js`
  ...
  - 두 장 보기 좌/우 개별 확대(`zoomLeft`/`zoomRight`) 및 페이지별 1회 클릭 확대 제한
  - 읽기 방향(`rtl`) 저장/복원(localStorage) + 버튼 라벨 동적 반영
  - 사이드바 활성 페이지 자동 스크롤을 중앙 정렬로 변경
  - 탐색기 검색창 추가(실시간 필터)
  - Enter 키 전체화면 토글 추가
  - 전체화면 3초 무입력 시 커서 숨김 + 페이지 이동 후 숨김 상태 유지
  - zip 내부 마지막 폴더(prefix) 저장/복원으로 재시작 복원 정확도 개선
  - About 모달(정보 버튼, ESC/오버레이 닫기) 추가
- `index.html` / `styles.css`
  - 시작 화면 법적 고지 문구 추가
  - About 모달 UI 추가
  - 툴바/제목 중앙 정렬 및 버튼 순서 정리
  - 야간 모드(웜 톤 필터) 버튼 추가
- `main.js`
