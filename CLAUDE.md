...
## 수동 점검 체크리스트 (취약점/성능/회귀)

수정 작업 전 "분석만" 할 때 사용하는 실사용 점검표.  
기준: 문제 재현 샘플이 없는 상태에서는 선제 최적화보다 증상 기반 패치를 우선.

### 1) 취약점 점검 (읽기 전용)

- [ ] `rename/delete/save`에서 `..`, 슬래시, 절대경로, 드라이브 문자 차단 확인
- [ ] 허용 루트 밖 경로에 대한 쓰기/삭제 거부 확인
- [ ] IPC 입력값(빈 문자열/비정상 타입/과도 길이) 안전 실패 확인
- [ ] zip 엔트리 수/이미지 크기 제한 초과 시 앱 멈춤 없이 실패 처리 확인
- [ ] 렌더러 문자열 렌더링이 `textContent` 기반인지 확인
- [ ] 사용자 에러 메시지에 절대경로 과다 노출이 없는지 확인

### 2) 성능 점검 (체감 + 계측)

- [ ] 대용량 zip에서 "목록 표시까지 시간" 허용 범위 확인
- [ ] 연속 페이지 넘김 시 프레임 드랍/멈춤 없는지 확인
- [ ] 두 장 보기 확대/축소/정렬 보정(`A/D`) 반응 속도 확인
- [ ] 20~30분 연속 사용 시 메모리 누적 증가 추세 확인
- [ ] 손상 zip 처리 후 다음 파일 열기 정상 복구 확인
- [ ] 중첩 zip(`inner.zip::image`) 탐색/읽기 지연 과다 여부 확인

### 3) 회귀 테스트 점검

- [ ] 한 장/두 장 전환 시 인덱스/줌 상태 꼬임 없는지
- [ ] RTL 모드에서 좌우 배치/이동 방향 정상인지
- [ ] 야간 모드 토글 + 재실행 유지 정상인지
- [ ] 전체화면 커서 자동 숨김/복귀 유지 정상인지
- [ ] 탐색기 검색/이름변경/삭제 후 목록 동기화 정상인지
- [ ] 진행상황/zip 내부 prefix 복원 정확한지

### 4) 권장 테스트 샘플셋

- [ ] `small`: 50~100장 일반 zip
- [ ] `large`: 1~2GB급 고해상도 zip
- [ ] `nested`: zip 안에 zip/cbz 포함
- [ ] `corrupted`: 일부 엔트리 손상 zip
- [ ] `edge-name`: 한글/특수문자/긴 파일명 포함 zip

### 5) 이슈 분류 기준

- **Blocker**: 크래시, 데이터 손실, 앱 멈춤
- **Major**: 핵심 기능 실패(열기/넘김/복원 불가), 심한 끊김
- **Minor**: 메시지/UX 불편, 일시적 지연
